...
def download_latest_release(repo_name, target_dir):
    ...
    if response.status_code == 200:
        ...
        success_text(f"Failed to get release information!", "red", 7000)
        #{response.text}
    return None
...
root.maxsize(375, 175)


# A canva so the text doesn't move everything 😭
text_canvas = tk.Canvas(root, width=325, height=20)
text_canvas.grid(row=8, column=0, columnspan=3)

successText_canvas = tk.Canvas(root, width=325, height=20)
successText_canvas.grid(row=5, column=0, columnspan=3)
...
success_label = tk.Label(successText_canvas, text="", fg="black", wraplength=730)

text_label = tk.Label(text_canvas, text="text", fg="black")
...
isZipCheckBox = tk.Checkbutton(root, text='', variable=isZip)
...
browse_button.grid(row=2, column=3, sticky="W")
...
successText_canvas.create_window(0, 0, anchor="nw", window=success_label)
